...
- strictly stop talking after giving the response
    ...
    def run(
        ...
        if self.tools:
            ...
            for _ in range(max_rounds):
                ...
                try:
                    cleaned_completion = re.sub(r'^```json|```$', '', completion, flags=re.MULTILINE).strip()
                    cleaned_completion = cleaned_completion.replace("\n", " ")
                    response_json = json.loads(str(cleaned_completion))
                    if list(response_json.keys()) == ['type', 'bot', 'content', 'status']:
